...
run_tunnel_test() { # <phase-prefix> <exe> <tunnel-env-value> <domain-to-check> <expected-body> [extra env=val ...]
    local prefix="$1" exe="$2" tunnel_env="$3" domain="$4" body="$5"; shift 5
    local extra_env=("$@")
    ...
    $SUDO env HOME="$RH" "${extra_env[@]}" "$exe" start --no-browser >"$work/start.out" 2>&1
    ...
    ( $SUDO env HOME="$RH" "${extra_env[@]}" "$exe" stop >/dev/null 2>&1 ) & local sp=$!
    ...
    if [ -z "$SEED" ]; then
        ...
        if [ "$st" != 200 ]; then
            ...
            if [ -z "$TOKEN" ]; then
                ...
                run_tunnel_test cloud "$NEW" "${TUNNEL}:80" "$TUNNEL" "$STAGING_BODY" \
