//! Individual diagnostic checks, grouped by the Binary, Network, DNS, TLS,
//! Auth, and System categories. Each returns `Some(Diagnostic)` when it finds a
//! problem and `None` when the check passes.
