...
Complete the implementation of the Port 0 + Virtual Overlay Network so that `PORT_ZERO=foo` + port 0 bindings allow seamless access via `foo.devenv.local` without any port conflicts, even across multiple worktrees.
