...
slug: task-21
status: done
title: 'Single canonical install.sh: publish as GitHub Release asset; portzero.cloud redirects to it'
milestones:
- milestone-1
...
`portzero.cloud/install.sh` becomes a redirect to it — a redirect is not a second
implementation.
...
- `portzero.cloud/install.sh` 302-redirects to that URL. `curl -fsSL` follows
  redirects, so `curl -fsSL https://portzero.cloud/install.sh | sh` works unchanged.
...
`portzero.cloud/install.sh` is served as a static file by nginx
...
- [ ] Add to `cloud/landing/nginx.conf` (exact-match wins over static serving):
      `location = /install.sh { return 302 https://github.com/LoumTechnologies/port-zero/releases/latest/download/install.sh; }`
...
- [ ] `portzero.cloud/install.sh` redirects there (server repo); old static copies
      deleted. (verify: `curl -fsSLI https://portzero.cloud/install.sh` shows 302)