...
they are **frozen snapshots**. Setting `os.environ["PZ_TUNNEL"]` at runtime
...
PZ_TUNNEL must already be set before the process started.
...
Read `PZ_TUNNEL` from the environment (read-only). Returns the value, or
`None` if unset.
...
# PZ_TUNNEL must already be set in the environment before running.
...
Run it as:
```bash
PZ_TUNNEL=myapp-mybranch.portzero.local python3 examples/http_server.py
```
