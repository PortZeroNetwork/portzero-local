...
> `PORT_ZERO` — the `.devenv.local` suffix.
>
...
> (`PORT_ZERO=hello.devenv.local:8080`), the overlay exposes the service on
...
You set `PORT_ZERO=hello.devenv.local:8080` **before** launching your service
...
export PORT_ZERO=hello.devenv.local:8080

# Option C: the port-zero-exec launcher from the SDKs
#   ../../sdks/direnv/port-zero-exec hello.devenv.local:8080 python3 server.py
```
...
# [server] PORT_ZERO=hello.devenv.local:8080
...
# PORT_ZERO=hello.devenv.local:8080
...
```bash
port-zero status    # shows the discovered .devenv.local service + VIP
